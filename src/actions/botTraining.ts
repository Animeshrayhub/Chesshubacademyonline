...
export async function startBotGameAction(data: {
  ...
  try {
    ...
    // Check if level is unlocked via points economy or coach override
    ...
    const maxUnlocked = Math.max(...(profile.unlocked_levels || [1]), 1);
    const unlocked = getUnlockedLevelsByPoints(userPoints, maxUnlocked, profile.coach_unlocked_levels || []);
    if (!unlocked.includes(data.botLevel)) {
      return {
        success: false,
        error: { message: `Level ${data.botLevel} is locked. Earn more training points or ask your coach to unlock it.` },
      };
    }
...
export async function finishBotGameAction(data: {
  ...
  try {
    ...
    // Check newly unlocked levels via points economy
    const currentPoints = calculateTrainingPoints(newWins, newDraws, newLosses, profile.puzzles_solved || 0);
    const coachUnlocked = profile.coach_unlocked_levels || [];
    const highestLevel = Math.max(...(profile.unlocked_levels || [1]), 1);
    const unlockedLevels = getUnlockedLevelsByPoints(currentPoints, highestLevel, coachUnlocked);
    const currentLevel = getCurrentLevelFromPoints(currentPoints);
